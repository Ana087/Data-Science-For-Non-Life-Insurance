...
plot.eda.agec <- ggplot.bar(data, agecar, "Car age")
...
# power of the car
data$powerc<-factor(data$powerc, levels=c('<66','66-110','>110'))
plot.eda.power <- ggplot.bar(data, data$powerc, "Power")


plot.eda.agec <- ggplot(data = data, aes(agecar)) + theme_bw() +
  geom_bar(stat = "identity", color = KULbg, 
           fill = KULbg, alpha = .5) +
  labs(x = "Car Age", y = "Relative frequency")
...
# adding the longitude and latitude coordinates from the post data
updated_data <-left_join(x=data,y=post_data, by=c('codposs'='CODPOSS'))
